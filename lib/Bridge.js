...
class Connection extends EventEmitter {
  ...
  _handleConnect() {
    ...
    this._readLine().then(line => {
      if (line.endsWith(EXPECTED_HELLO)) {
        ...
        throw Connection.unexpectedHelloError
      }
    })
  }
  ...
  _handleTimeout() {
    this.socket.end()
    this.emit('error', Connection.timeoutError)
  }
...
Connection.connectError = Error('unable to connect')
Connection.timeoutError = Error('timeout')
Connection.unexpectedHelloError = Error('unexpected hello response from bridge')
