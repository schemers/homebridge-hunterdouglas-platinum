...
class Config {
  constructor(lines) {
    ...
    this.rooms = new Map() // rooms['room-id'] = Map()

    for (const line of lines) {
      if ((match = line.match(/^\$firm\d+-(.*)$/))) {
        this.softwareVersion = match[1]
      } else if ((match = line.match(/^\$MAC0x(.*)-$/))) {
        this.serialNumber = match[1]
      } else if ((match = line.match(/^\$LEDl(\d\d\d)-$/))) {
        console.log(match)
        this.ledBrightness = Number(match[1]) / 255
      } else if ((match = line.match(/^\$ct(.*)$/))) {
        this.homeName = match[1]
      } else if ((match = line.match(/^\$cr(\d\d)-[^-]+-[^-]-(.*)$/))) {
        const roomId = match[1]
        const roomName = match[2]
        this.rooms[roomId] = { id: roomId, name: roomName }
        s
      }
    }
  }
